...
class Pengembalian extends CI_Controller
{
    ...
    public function index()
    {
        if (isset($_GET['filter']) && !empty($_GET['filter'])) { // Cek apakah user telah memilih filter dan klik tombol tampilkan
            ...
            if ($filter == '1') { // Jika filter nya 1 (per tanggal)
                ...
                $label = 'Data Pengembalian Tanggal ' . date('d-m-y', strtotime($tgl));
                $url_export = 'admin/pengembalian/export?filter=1&tanggal=' . $tgl;
                $transaksi = $this->m_pengembalian->view_by_date($tgl); // Panggil fungsi view_by_date yang ada di TransaksiModel
            } else if ($filter == '2') { // Jika filter nya 2 (per bulan)
                $bulan = $_GET['bulan'];
                $tahun = $_GET['tahun'];
                $nama_bulan = array('', 'Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni', 'Juli', 'Agustus', 'September', 'Oktober', 'November', 'Desember');

                $label = 'Data Pengembalian Bulan ' . $nama_bulan[$bulan] . ' ' . $tahun;
                $url_export = 'admin/pengembalian/export?filter=2&bulan=' . $bulan . '&tahun=' . $tahun;
                $transaksi = $this->m_pengembalian->view_by_month($bulan, $tahun); // Panggil fungsi view_by_month yang ada di TransaksiModel
            } else { // Jika filter nya 3 (per tahun)
                $tahun = $_GET['tahun'];

                $label = 'Data Pengembalian Tahun ' . $tahun;
                $url_export = 'admin/pengembalian/export?filter=3&tahun=' . $tahun;
                $transaksi = $this->m_pengembalian->view_by_year($tahun); // Panggil fungsi view_by_year yang ada di TransaksiModel
            }
        } else { // Jika user tidak mengklik tombol tampilkan
            $label = 'Semua Data Pengembalian';
            $url_export = 'admin/pengembalian/export';
            $transaksi = $this->m_pengembalian->view_all(); // Panggil fungsi view_all yang ada di TransaksiModel
        }

        $data['label'] = $label;
        $data['url_export'] = base_url('index.php/' . $url_export);
        $data['transaksi'] = $transaksi;
        $data['option_tahun'] = $this->m_pengembalian->option_tahun();
    ...
    public function export()
    {
        ...
        $excel->getProperties()->setCreator('Perpustakaan Indramayu')
            ->setLastModifiedBy('Perpustakaan Pengembalian')
        ...
        $excel->getActiveSheet()->setCellValue('A1', "PENGEMBALIAN"); // Set kolom A1 dengan tulisan "DATA MEMBER"
        ...
        $excel->getActiveSheet()->setTitle("Laporan Data Pengembalian");
