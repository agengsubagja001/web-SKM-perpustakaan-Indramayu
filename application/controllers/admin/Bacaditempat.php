...
class Bacaditempat extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('admin/m_bacaditempat');
    }
    ...
    public function index()
    {
        if (isset($_GET['filter']) && !empty($_GET['filter'])) { // Cek apakah user telah memilih filter dan klik tombol tampilkan
            ...
            if ($filter == '1') { // Jika filter nya 1 (per tanggal)
                ...
                $label = 'Data Baca di tempat Tanggal ' . date('d-m-y', strtotime($tgl));
                $url_export = 'admin/bacaditempat/export?filter=1&tanggal=' . $tgl;
                ...
                $label = 'Data Baca di tempat Bulan ' . $nama_bulan[$bulan] . ' ' . $tahun;
                $url_export = 'admin/bacaditempat/export?filter=2&bulan=' . $bulan . '&tahun=' . $tahun;
                $transaksi = $this->m_bacaditempat->view_by_month($bulan, $tahun); // Panggil fungsi view_by_month yang ada di TransaksiModel
            } else { // Jika filter nya 3 (per tahun)
                $tahun = $_GET['tahun'];

                $label = 'Data Baca di tempat Tahun ' . $tahun;
                $url_export = 'admin/bacaditempat/export?filter=3&tahun=' . $tahun;
                $transaksi = $this->m_bacaditempat->view_by_year($tahun); // Panggil fungsi view_by_year yang ada di TransaksiModel
            }
        } else { // Jika user tidak mengklik tombol tampilkan
            $label = 'Semua Data Baca di tempat';
            $url_export = 'admin/bacaditempat/export';
            $transaksi = $this->m_bacaditempat->view_all(); // Panggil fungsi view_all yang ada di TransaksiModel
        }

        $data['label'] = $label;
        $data['url_export'] = base_url('index.php/' . $url_export);
        $data['transaksi'] = $transaksi;
        $data['option_tahun'] = $this->m_bacaditempat->option_tahun();
        $this->load->view('admin/bacaditempat', $data);
    }
    ...
    public function export()
    {
        ...
        $excel->getProperties()->setCreator('Perpustakaan Indramayu')
            ->setLastModifiedBy('Perpustakaan Baca di tempat')
            ->setTitle("Data Baca di tempat")
            ->setSubject("Baca di tempat")
            ->setDescription("Laporan Semua Data Baca di tempat")
            ->setKeywords("Data Baca di tempat");
        ...
        $excel->getActiveSheet()->setTitle("Laporan Data Baca di tempat");
        $excel->getActiveSheet();

        // Proses file excel
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment; filename="Data Bacaditempat.xlsx"'); // Set nama file excel nya
