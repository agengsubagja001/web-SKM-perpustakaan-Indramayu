...
class Internet extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('admin/m_internet');
    }
    ...
    public function index()
    {
        if (isset($_GET['filter']) && !empty($_GET['filter'])) { // Cek apakah user telah memilih filter dan klik tombol tampilkan
            ...
            if ($filter == '1') { // Jika filter nya 1 (per tanggal)
                ...
                $label = 'Data Internet Tanggal ' . date('d-m-y', strtotime($tgl));
                $url_export = 'admin/internet/export?filter=1&tanggal=' . $tgl;
                $transaksi = $this->m_internet->view_by_date($tgl); // Panggil fungsi view_by_date yang ada di TransaksiModel
            } else if ($filter == '2') { // Jika filter nya 2 (per bulan)
                $bulan = $_GET['bulan'];
                $tahun = $_GET['tahun'];
                $nama_bulan = array('', 'Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni', 'Juli', 'Agustus', 'September', 'Oktober', 'November', 'Desember');

                $label = 'Data Internet Bulan ' . $nama_bulan[$bulan] . ' ' . $tahun;
                $url_export = 'admin/internet/export?filter=2&bulan=' . $bulan . '&tahun=' . $tahun;
                $transaksi = $this->m_internet->view_by_month($bulan, $tahun); // Panggil fungsi view_by_month yang ada di TransaksiModel
            } else { // Jika filter nya 3 (per tahun)
                $tahun = $_GET['tahun'];

                $label = 'Data Internet Tahun ' . $tahun;
                $url_export = 'admin/internet/export?filter=3&tahun=' . $tahun;
                $transaksi = $this->m_internet->view_by_year($tahun); // Panggil fungsi view_by_year yang ada di TransaksiModel
            }
        } else { // Jika user tidak mengklik tombol tampilkan
            $label = 'Semua Data Internet';
            $url_export = 'admin/internet/export';
            $transaksi = $this->m_internet->view_all(); // Panggil fungsi view_all yang ada di TransaksiModel
        }

        $data['label'] = $label;
        $data['url_export'] = base_url('index.php/' . $url_export);
        $data['transaksi'] = $transaksi;
        $data['option_tahun'] = $this->m_internet->option_tahun();
        $this->load->view('admin/internet', $data);
    }
    ...
    public function export()
    {
        ...
        $excel->getProperties()->setCreator('Perpustakaan Indramayu')
            ->setLastModifiedBy('Perpustakaan Internet')
            ->setTitle("Data Baca di Internet")
            ->setSubject("Internet")
            ->setDescription("Laporan Semua Data Internet")
            ->setKeywords("Data Internet");
        ...
        $excel->getActiveSheet()->setCellValue('A1', "INTERNET"); // Set kolom A1 dengan tulisan "DATA MEMBER"
        ...
        $excel->getActiveSheet()->setTitle("Laporan Data Internet");
        $excel->getActiveSheet();

        // Proses file excel
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment; filename="Data Internet.xlsx"'); // Set nama file excel nya
